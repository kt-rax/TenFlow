...
X_input,y_input = mnist['data'].astype(np.float32).to_numpy(),mnist['target'].astype(np.float32).to_numpy() #.astype(np.int8)
...
sess,cost_history,accuracy_history = run_logistic_model(learning_r=0.01,training_epochs=500,train_obs=xtrain,train_lables=ytrain,debug=False)

plot2fig(cost_history,accuracy_history,title=' 6.9 cost and accuracy',legend_str=['cost J','accuracy'])
